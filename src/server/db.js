...
const dbApi = {
  ...
  addQuestion: (quizId, text, opt_a, opt_b, opt_c, opt_d, correct_opt, image, imageWidth, imageHeight) => {
    return new Promise((resolve, reject) => {
      db.run(`INSERT INTO questions (quiz_id, text, opt_a, opt_b, opt_c, opt_d, correct_opt, image, image_width, image_height) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
              [quizId, text, opt_a, opt_b, opt_c, opt_d, correct_opt, image, imageWidth, imageHeight], function(err) {
        if (err) reject(err);
        else resolve(this.lastID);
      });
    });
  ...
  updateQuestion: (id, text, opt_a, opt_b, opt_c, opt_d, correct_opt, image, imageWidth, imageHeight) => {
    return new Promise((resolve, reject) => {
      db.run(`UPDATE questions SET text = ?, opt_a = ?, opt_b = ?, opt_c = ?, opt_d = ?, correct_opt = ?, image = ?, image_width = ?, image_height = ? WHERE id = ?`,
        [text, opt_a, opt_b, opt_c, opt_d, correct_opt, image, imageWidth, imageHeight, id], function(err) {
          if (err) reject(err);
          else resolve(this.changes);
        });
    });
